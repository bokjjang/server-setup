...
```bash
# 방법 1: 리포 클론 후 실행
git clone https://github.com/bokjjang/server-setup.git ~/server-setup
cd ~/server-setup && bash install.sh

# 방법 2: 원라이너 (리포 자동 클론)
curl -fsSL https://raw.githubusercontent.com/bokjjang/server-setup/main/install.sh | bash
```

### 설치 모드

스크립트 실행 시 대화형 UI(whiptail)로 설치 모드를 선택한다.

| 모드 | 설명 |
|------|------|
| 전체 설치 | 새 서버 초기 세팅. 모든 항목을 한번에 설치 |
| 선택 설치 | 체크박스에서 원하는 항목만 선택하여 설치 |
| dotfiles만 | 프로그램 설치 없이 설정 파일 심볼릭 링크만 연결 |
...
공통 설정은 이 리포에서 관리하고, 서버별 차이는 `.local` 파일로 관리한다.

```bash
# 서버에서 직접 생성 (gitignore 대상)
~/.zshrc.local          # 서버별 PATH, alias 등
```

`.zshrc` 맨 끝에서 `.zshrc.local`을 자동으로 source한다.
...
gpakosz는 기본 tmux 키바인딩을 변경한다.
특히 윈도우 이동 키 `prefix + n/p`를 해제하고 `prefix + Ctrl+h/Ctrl+l`로 변경한다.
`n/p`가 필요하면 `.tmux.conf.local`에 `bind n next-window`, `bind p previous-window`를 추가한다.
...
- 마우스 스크롤은 tmux 자체 버퍼만 스크롤됨 (tmux 진입 전 출력은 볼 수 없음)
- 따라서 서버 접속 후 바로 tmux에 들어가는 습관이 좋다

**패널 이동 (vim-tmux-navigator)**
- `Ctrl + h/j/k/l` → prefix 없이 바로 패널 이동
- 현재 패널에서 vim이 실행 중이면 vim 분할창 이동으로 동작
- vim이 없으면 tmux 패널 이동으로 동작
- **vim 분할창 ↔ tmux 패널 경계도 자유롭게 넘나든다**
...
| `Ctrl + h/j/k/l` | 패널 이동 (prefix 없이 바로, vim-tmux-navigator) |
...
vim을 열면 이 순서로 설정이 로드된다:
1. `~/.vimrc` → amix의 `~/.vim_runtime/vimrc.vim`을 source
2. amix 기본 설정 적용 (검색, 색상, 파일 타입 등)
3. `~/.vim_runtime/my_configs.vim` 로드 (dotfiles에서 심볼릭 링크된 개인 설정)

### vim-tmux-navigator

vim 플러그인으로, `~/.vim/pack/plugins/start/`에 설치된다.
`Ctrl+hjkl`을 누르면 vim 분할창 경계인지 tmux 패널 경계인지 자동 감지한다.
tmux 설정(.tmux.conf.local)의 `if-shell` 바인딩과 함께 동작한다.

별도 매핑이 불필요하다. 플러그인이 자동으로 `Ctrl+hjkl`을 처리한다.
tmux 쪽에서도 `if-shell`로 vim 실행 여부를 감지하여 키를 전달하거나 패널을 이동한다.
